...
async function receiveGiftWrapped(): Promise<string> {
  await initRelay();
  return new Promise((resolve, reject) => {
    const sub = relay.sub([{ kinds: [1059], authors: [senderPubKey] }]);
    sub.on('event', async (evt: Event) => {
      if (evt.tags.some(t => t[0] === 'p' && t[1] === receiverPubKey)) {
        sub.unsub();
        const decrypted = await nip04.decrypt(receiverPrivKey, senderPubKey, evt.content);
        resolve(decrypted);
      }
    });
    sub.on('eose', () => { sub.unsub(); reject(new Error('No message')); });
    setTimeout(() => { sub.unsub(); reject(new Error('Timeout')); }, 10000);
  });
}
  ...
  return (
    <div className="p-6 space-y-6 max-w-2xl mx-auto">
      ...
      <div className="border p-4 rounded">
        ...
        {invoice && <pre className="mt-2 bg-gray-100 p-2">{invoice}</pre>}
        <button
      ...
      <div className="border p-4 rounded">
        ...
        {token && <textarea className="border p-2 w-full mb-2" rows={2} readOnly value={token} />}        
        <textarea
          className="border p-2 w-full mb-2"
          rows={2}
        ...
        <button
          ...
          onClick={async () => {
            try {
              ...
              setPubkey(''); setLock(''); setToken(''); setSignature('');
            } catch (e: any) {
              toast.error(e.message);
            }
          }}
